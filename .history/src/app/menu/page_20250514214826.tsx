      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row gap-8">
            ...
            <ul className="md:space-y-4 md:px-4 flex flex-row md:flex-col gap-2 md:gap-0 px-2 w-full">
              {categories.map(cat => (
                <li key={cat.key} className="flex-1 min-w-[120px] md:min-w-0">
                  <button
                    className={`w-full h-12 md:h-14 text-center flex items-center justify-center px-4 py-2 rounded-lg font-semibold transition-colors duration-200 text-lg md:text-base ${selectedCategory === cat.key ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700 hover:bg-green-50'}`}
          ...
          <div className="flex-1 md:ml-0 md:mt-0 mt-6">
            <h2 className="text-3xl font-bold text-center mb-8" style={{ color: '#204B2A' }}>{categories.find(c => c.key === selectedCategory)?.label}</h2>
