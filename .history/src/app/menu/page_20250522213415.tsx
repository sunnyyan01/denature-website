...
export default function MenuPage() {
  ...
  return (
    <main style={{ background: '#F8F6F1' }}>
              ...
              {filteredMenus.map((item, idx) => (
                ...
                >
                  ...
                  <div className="flex-1 flex flex-col p-5">
                    ...
                    <button
                      className="block mt-2 text-center text-gray-400 text-xs hover:text-gray-600 transition-colors"
                      onClick={() => setModalMeal(item)}
                    >
                      More info
                    </button>
                  </div>
      ...
      {/* 购物车商品图片和菜单实物图片一致的JS */}
      <Script id="shopify-cart-image-replace" strategy="afterInteractive">
        {`
          const productImageMap = {
            "Teriyaki Udon with Grilled Tofu": "/images/menu/teriyaki-tofu-udon.jpg",
            "Pan-Fried Veggie Dumplings with Fresh Salad": "/images/menu/veggie-dumplings.jpg",
            "Korean Veggie Kimbap": "/images/menu/veggie-kimbap.jpg",
            "Rainbow Fried Rice with Plant Nuggets": "/images/menu/veggie-fried-rice.jpg",
            "Inari Tofu Sushi with Garden Salad": "/images/menu/inari-sushi.jpg",
            "Millet Cake with Sweet-Sour Veggie Ribs": "/images/menu/millet-cake.jpg",
            "Lentil Patties with Brown Rice": "/images/menu/lentil-patties-veg.jpg",
            "Fire Veggie Noodles with Plant Chicken": "/images/menu/fire-noodles.jpg",
            "Sushi Rice Balls with Plant Nuggets": "/images/menu/sushi-balls.jpg",
            "Sweet & Sour Veggie Ribs with Millet Rice": "/images/menu/sweet-sour-ribs.jpg"
          };
          function replaceCartImages() {
            document.querySelectorAll('.shopify-buy__cart__item').forEach(function(item) {
              const titleEl = item.querySelector('.shopify-buy__cart__item__title');
              const imgEl = item.querySelector('.shopify-buy__cart__item__image');
              if (titleEl && imgEl) {
                const name = titleEl.textContent.trim();
                const imgUrl = productImageMap[name];
                if (imgUrl) {
                  imgEl.style.background = 'url(' + imgUrl + ') center center/cover no-repeat';
