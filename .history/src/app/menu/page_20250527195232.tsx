...
export default function MenuPage() {
  ...
  const [modalComp, setModalComp] = useState<any | null>(null);

  // 筛选菜品
  const filteredMenus = selectedCategory === 'vegetarian' ? vegetarianMenus : glutenFreeMenus;

  useEffect(() => {
    window.setReactModalMeal = setModalMeal;
    window.setReactModalComp = setModalComp;
  }, []);
  ...
  return (
    <main style={{ background: '#F8F6F1' }}>
                ...
                return (
                  <div key={item.id} data-meal-name={item.name} className="bg-white rounded-xl shadow-md overflow-hidden w-full max-w-xl h-full flex flex-col">
                    ...
                    <div className="flex-1 flex flex-col p-5">
                      ...
                      <Script id={`shopify-buy-button-${item.id}`} strategy="afterInteractive">
                        {`
                          (function () {
                            ...
                            function ShopifyBuyInit() {
                              ...
                              ShopifyBuy.UI.onReady(client).then(function (ui) {
                                var comp = ui.createComponent('product', {
                                  id: '${item.shopifyId}',
                                  node: document.getElementById('product-component-${item.id}'),
                                ...
                                // 拦截按钮点击，弹出时间选择弹窗
                                setTimeout(function() {
                                  var btn = document.querySelector(`#product-component-${item.id} .shopify-buy__btn`);
                                  if (btn) {
                                    btn.addEventListener('click', function(e) {
                                      e.preventDefault();
                                      window.__showTimeSelectModal && window.__showTimeSelectModal('${item.name}', comp);
                                    });
                                  }
                                }, 1000);
                              });
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6 relative">
            <button
              ...
              onClick={() => { setModalMeal(null); setSelectedTime(''); setModalComp(null); }}
              aria-label="Close"
            >
              ×
            </button>
            ...
            <button
              ...
              onClick={() => {
                if (modalComp && selectedTime) {
                  modalComp.props.options.product.events.addVariantToCart = function(product) {
                    product.model.selectedOptions.push({
                      name: 'Delivery Time',
                      value: selectedTime
                    });
                  };
                  modalComp.addVariantToCart();
                }
                setModalMeal(null);
                setSelectedTime('');
                setModalComp(null);
      ...
      {/* 全局方法，供Shopify按钮调用弹窗 */}
      <Script id="show-time-select-modal" strategy="afterInteractive">
        {`
          window.__showTimeSelectModal = function(mealName, comp) {
            if (window.setReactModalMeal && window.setReactModalComp) {
              window.setReactModalMeal(mealName);
              window.setReactModalComp(comp);
            }
          }
        `}
      </Script>
