...
function getMenuPeriodAndDay(dateStr) {
  const start = new Date(MENU_START_DATE);
  const selected = new Date(dateStr);
  const days = Math.floor((selected - start) / (1000 * 60 * 60 * 24));
...
export default function MenuPage() {
  ...
  const todayMeal = selectedDate && menuCycle[weekType]?.[dayOfWeek];
                  ...
                  >
                    ...
                    <div className="flex-1 flex flex-col p-5">
                      ...
                      <button
                        ...
                        className={isTodayMeal ? 'btn-primary' : 'btn-disabled'}
                      >
                        {isTodayMeal ? 'Add to cart' : '不可购买'}
                      </button>
                    </div>
                  </div>
                );
