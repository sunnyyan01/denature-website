            ...
            <ul className="md:space-y-4 md:px-4 flex flex-row md:flex-col gap-2 md:gap-0 px-2 w-full">
              {categories.map(cat => (
                <li key={cat.key} className="flex-1 min-w-[120px] md:min-w-0">
                  <button
                    className={`w-full h-16 text-center flex items-center justify-center px-4 py-2 rounded-lg font-medium transition-colors duration-200 text-lg md:text-base border-none bg-[#1F442E] text-white hover:bg-[#18351F]`}
                    style={{ borderRadius: 8, fontWeight: 500, fontSize: 18 }}
                    ...
                    <div className="space-y-2 mb-4 flex-1">
                      {item.details.map((detail, index) => (
                        <p key={index} style={{ color: '#4D4D4D', fontSize: 14 }}>
                          {detail.includes('Health Rating') ? detail.replace('Health Rating:', '🟢 85% 🟠 10% 🔴 5%') :
                            detail.includes('NSW') ? <span>✅ Everyday</span> : detail}
                        </p>
                      ))}
                    </div>
                  </div>
