...
export default function MenuPage() {
  ...
  const [pendingComp, setPendingComp] = useState<any | null>(null);

  const filteredMenus = selectedCategory === 'vegetarian' ? vegetarianMenus : glutenFreeMenus;

  // 记录每个商品的 Buy Button 组件实例
  const buyButtonComps: Record<number, any> = {};

  useEffect(() => {
    // 动态加载 Shopify Buy Button UI
      ...
      if (window.ShopifyBuy) {
        ...
        window.ShopifyBuy.UI.onReady(client).then(function (ui: any) {
          filteredMenus.forEach((item) => {
            const node = document.getElementById(`product-component-${item.id}`);
            if (node && !node.hasChildNodes()) {
              const comp = ui.createComponent('product', {
                id: item.shopifyId,
                node,
                moneyFormat: '%24%7B%7Bamount%7D%7D',
                options: {
                  product: {
                    contents: { title: false, button: true },
                    styles: {
                      product: { boxShadow: 'none', border: 'none' },
                      button: {
                        background: '#199E5B',
                        color: '#fff',
                        borderRadius: '8px',
                    ...
                    events: {
                      addVariantToCart: function(product: any) {
                        // 这里什么都不做，交给自定义弹窗处理
                        return false;
                      },
                      click: (product: any, e: any) => {
                        e.preventDefault();
                        setModalMeal(item);
                        setSelectedTime('');
                        setPendingComp(buyButtonComps[item.id]);
                        return false;
                      },
                    },
                  },
  ...
  // 处理弹窗确认
  const handleConfirm = () => {
    if (pendingComp && selectedTime) {
      // 注入自定义属性
      pendingComp.props.options.product.events.addVariantToCart = function(product: any) {
        product.model.selectedOptions.push({
          name: 'Delivery Time',
          value: selectedTime,
        });
      };
      pendingComp.addVariantToCart();
    }
    setModalMeal(null);
    setSelectedTime('');
    setPendingComp(null);
  ...
  return (
    <main style={{ background: '#F8F6F1' }}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6 relative">
            <button
              ...
              onClick={() => { setModalMeal(null); setSelectedTime(''); setPendingComp(null); }}
              aria-label="Close"
            >
              ×
            </button>
            ...
            >
              ...
              {modalMeal && getDeliveryOptions(modalMeal.name).map(opt => (
                <option key={opt} value={opt}>{opt}</option>
              ))}
            </select>
            <button
              className="shopify-buy__btn w-full py-3 rounded-lg text-white font-bold text-base transition-colors duration-200"
              style={{ background: '#199E5B', marginTop: 8, boxShadow: '0 2px 8px rgba(25,158,91,0.08)' }}
      ...
      {/* 动态加载 ShopifyBuy SDK 脚本 */}
      <Script src="https://sdks.shopifycdn.com/buy-button/latest/buy-button-storefront.min.js" strategy="afterInteractive" />
    </main>
  );
} 