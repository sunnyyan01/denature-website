...
export default function MenuPage() {
  ...
  const buyButtonComps = useRef<Record<number, any>>({});
        ...
        window.ShopifyBuy.UI.onReady(client).then(function (ui: any) {
          filteredMenus.forEach((item) => {
            ...
            if (node && !node.hasChildNodes()) {
              ...
              buyButtonComps.current[item.id] = comp;
            }
          });
        });
      }
  ...
  // 处理弹窗确认
  const handleConfirm = () => {
    if (pendingId && selectedTime) {
      const comp = buyButtonComps.current[pendingId];
      if (comp) {
        comp.updateConfig('product', {
          lineItemProperties: {
            'Delivery Time': selectedTime
          }
        });
        // 自动点击 Add to cart 按钮
        const btn = document.querySelector(`#product-component-${pendingId} .shopify-buy__btn`) as HTMLElement;
        if (btn) btn.click();
      }
    }
    setModalMeal(null);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-lg max-w-md w-full p-6 relative">
            <button
              ...
              onClick={() => { setModalMeal(null); setSelectedTime(''); setPendingId(null); }}
              aria-label="Close"
            >
              ×
            </button>
