...
export default function MenuPage() {
  ...
  useEffect(() => {
    // Dynamically load ShopifyBuy SDK
          ...
          <div className="flex-1 md:ml-0 md:mt-0 mt-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center items-stretch">
              {filteredMenus.map((item) => (
                <div key={item.id} className="bg-white rounded-xl shadow-md overflow-hidden w-full max-w-xl h-full flex flex-col">
                  ...
                  <div className="flex-1 flex flex-col p-5">
                    ...
                    <p style={{ color: '#4D4D4D', fontSize: 14 }}>{item.description}</p>
                    <p style={{ color: '#4D4D4D', fontSize: 14 }}>Main Grain: {item.mainGrain}</p>
                    <p style={{ color: '#4D4D4D', fontSize: 14 }}>Protein: {item.protein}</p>
                    <p style={{ color: '#4D4D4D', fontSize: 14 }}>Vegetables: {item.vegetables}</p>
                    <p style={{ color: '#4D4D4D', fontSize: 14 }}>NSW Category: {item.nswCategory}</p>
                  </div>
                </div>
              ))}
